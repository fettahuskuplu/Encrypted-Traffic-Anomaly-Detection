...
class TrafficLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, num_classes):
        ...
        # LSTM Katmanı (batch_first=True verinin boyutunu [Batch, Seçilen_Zaman_Adımı, Özellik_Sayısı] olarak bekler)
        self.lstm = nn.LSTM(input_dim, hidden_dim, num_layers, batch_first=True)

        # Tam Bağlantılı Çıkış Katmanı (Sınıflandırma için)
        self.fc = nn.Linear(hidden_dim, num_classes)

    def forward(self, x):
        # Hidden ve Cell durumlarının sıfırlanması
        ...
        # Son zaman adımının çıktısını alıp sınıflandırıcıya gönderiyoruz
        out = self.fc(out[:, -1, :])
        return out
...
def train_and_evaluate_lstm():
    ...
    # Sınıf sayısını dinamik olarak tespit etme (İkili veya Çoklu Sınıf uyumluluğu için)
    ...
    # MÜHENDİSLİK ADIMI: LSTM veriyi 3 boyutlu ister -> [Örnek Sayısı, Zaman Adımı, Özellik Sayısı]
    # Veriler ön işleme aşamasında split edildiği için zaman adımını (time_step) 1 alarak boyut genişletiyoruz.
    X_train_3d = np.expand_dims(X_train, axis=1)
    X_val_3d = np.expand_dims(X_val, axis=1)
    X_test_3d = np.expand_dims(X_test, axis=1)

    print(f"    - LSTM için Yeni Eğitim Boyutu: {X_train_3d.shape}")
    print(f"    - LSTM için Yeni Test Boyutu: {X_test_3d.shape}")
    ...
    # DataLoader Veri Yükleyicileri
    batch_size = 2048
    train_loader = DataLoader(TensorDataset(X_train_tensor, y_train_tensor), batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(TensorDataset(X_val_tensor, y_val_tensor), batch_size=batch_size, shuffle=False)
    test_loader = DataLoader(TensorDataset(X_test_tensor, y_test_tensor), batch_size=batch_size, shuffle=False)

    # Cihaz Seçimi (GPU/CUDA optimizasyonu)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[2/5] Model donanım hızlandırıcı cihazı (Device): {device}")

    # Hiperparametreler ve Model Yapılandırması
    input_dim = X_train.shape[1]  # Özellik sayısı
    hidden_dim = 64  # LSTM hücre boyutu
    num_layers = 2  # Ardışık LSTM katman sayısı

    model = TrafficLSTM(input_dim, hidden_dim, num_layers, num_classes).to(device)

    # Kayıp fonksiyonu ve ağırlıklandırma/optimizasyon kurgusu
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.001)

    # Erken Durdurma (Early Stopping) Ayarları
    num_epochs = 50
    patience = 5
    ...
    # --- NİHAİ TEST VE PERFORMANS RAPORLAMASI ---
    ...
    # Sınıf isimlerini dosyadan okuma kurgusu
    sinif_haritasi = {}
